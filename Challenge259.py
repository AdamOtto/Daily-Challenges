'''
Ghost is a two-person word game where players
alternate appending letters to a word.
The first person who spells out a word, or creates a prefix
for which there is no possible continuation, loses.
...
Given a dictionary of words, determine the letters 
Player1 should start with, such that with optimal play they cannot lose.

For example, if the dictionary is
["cat", "calf", "dog", "bear"]
the winning start letter would be b and c.

Player1: c
Player2: a (player2 only has one option, so must play 'a')
Player1: l (picking t would lose, so Player1 picks 'l')
Player2: f [loses]
...
Another similar example, the dictionary is
["cat", "calf", "con", "bear"]
The only winning move is to start with b

Player1: c
Player2: o (Player2 can either pick 'a' or 'o'. Only 'o' is optimal for Player2)
Player1: n [loses]
